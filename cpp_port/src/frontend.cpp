...
function renderStrategyNote(strategy){const mode=String(strategy||"raw_delta");strategyNote.textContent=mode==="legacy_pid"?"Legacy PID uses the dedicated PID.cpp-style controller path and ignores Velocity Beta, Lead Time, ego-motion compensation, anti-windup, derivative smoothing, output limit, and PID settle knobs.":"Raw/raw_delta use the modern PID + feedforward path. Legacy PID knobs are ignored unless Tracking Strategy is set to Legacy PID.";}
...
function renderStatus(s){const settle=s.target_found?`${s.pid_settled?"settled":"gating"} ${Number(s.pid_settle_error_metric_px||0).toFixed(1)}/${Number(s.pid_settle_threshold_px||0).toFixed(1)}`:"n/a";runtime.textContent=`Runtime ${s.running?"running":"stopped"} | mode ${s.mode_label} | aim ${s.aim_mode_label||s.aimmode_label} | preview ${s.debug_preview_enable?"ON":"OFF"} | overlay ${s.debug_overlay_enable?"ON":"OFF"} | F8 ${s.triggerbot_enable?"ON":"OFF"} | recoil ${s.recoil_mode} | profile ${s.selected_profile_name||s.selected_profile_id||"none"} | target ${s.target_found?"locked":"none"} | speed ${Number(s.target_speed).toFixed(1)} | settle ${settle} | cmd (${s.aim_dx}, ${s.aim_dy})`;renderStrategyNote(s.tracking_strategy);renderModeToggles(s);renderRecoilStatus(s);}
