...
std::string buildConfigJson(const RuntimeConfig& cfg, const std::uint64_t version, const std::uint64_t reset_token) {
    ...
    oss << "{"
        ...
        << "\"detection_dampening_enable\":" << (cfg.detection_dampening_enable ? "true" : "false") << ","
        << "\"detection_dampening_stable_frames\":" << cfg.detection_dampening_stable_frames << ","
        ...
        << "\"predictive_pid_kp\":" << cfg.predictive_pid_kp << ","
        << "\"predictive_pid_ki\":" << cfg.predictive_pid_ki << ","
        << "\"predictive_pid_kd\":" << cfg.predictive_pid_kd << ","
        << "\"predictive_pid_pred_weight_x\":" << cfg.predictive_pid_pred_weight_x << ","
        << "\"predictive_pid_pred_weight_y\":" << cfg.predictive_pid_pred_weight_y << ","
        << "\"predictive_pid_init_scale\":" << cfg.predictive_pid_init_scale << ","
        << "\"predictive_pid_ramp_time_s\":" << cfg.predictive_pid_ramp_time_s << ","
        << "\"predictive_pid_integral_limit\":" << cfg.predictive_pid_integral_limit << ","
        << "\"predictive_pid_derivative_limit\":" << cfg.predictive_pid_derivative_limit << ","
        << "\"predictive_pid_output_limit\":" << cfg.predictive_pid_output_limit << ","
        << "\"predictive_pid_velocity_alpha\":" << cfg.predictive_pid_velocity_alpha << ","
        << "\"predictive_pid_acceleration_alpha\":" << cfg.predictive_pid_acceleration_alpha << ","
        << "\"predictive_pid_max_velocity_px_s\":" << cfg.predictive_pid_max_velocity_px_s << ","
        << "\"predictive_pid_max_acceleration_px_s\":" << cfg.predictive_pid_max_acceleration_px_s << ","
        << "\"predictive_pid_reverse_gate_px\":" << cfg.predictive_pid_reverse_gate_px << ","
        << "\"predictive_pid_reverse_scale\":" << cfg.predictive_pid_reverse_scale << ","
        << "\"predictive_pid_prediction_error_scale\":" << cfg.predictive_pid_prediction_error_scale << ","
        << "\"predictive_pid_prediction_min_px\":" << cfg.predictive_pid_prediction_min_px << ","
        << "\"predictive_pid_prediction_max_px\":" << cfg.predictive_pid_prediction_max_px << ","
        << "\"predictive_pid_latency_comp_enable\":" << (cfg.predictive_pid_latency_comp_enable ? "true" : "false") << ","
        << "\"predictive_pid_latency_auto_enable\":" << (cfg.predictive_pid_latency_auto_enable ? "true" : "false") << ","
        << "\"predictive_pid_latency_bias_s\":" << cfg.predictive_pid_latency_bias_s << ","
        << "\"predictive_pid_latency_max_s\":" << cfg.predictive_pid_latency_max_s << ","
        << "\"predictive_pid_deadzone_enable\":" << (cfg.predictive_pid_deadzone_enable ? "true" : "false") << ","
        << "\"predictive_pid_deadzone_enter_px\":" << cfg.predictive_pid_deadzone_enter_px << ","
        ...
        << "\"detection_box_scale\":" << cfg.detection_box_scale << ","
        << "\"kalman_prediction_enable\":" << (cfg.kalman_prediction_enable ? "true" : "false") << ","
        ...
        << "\"triggerbot_arm_min_x_px\":" << cfg.triggerbot_arm_min_x_px << ","
        << "\"triggerbot_arm_min_y_px\":" << cfg.triggerbot_arm_min_y_px << ","
        ...
        << "\"mouse_move_suppress_on_fire_debug\":" << (cfg.mouse_move_suppress_on_fire_debug ? "true" : "false") << ","
...
std::string buildStatusJson(const RuntimeConfig& cfg, const SharedState& shared_state) {
    ...
    oss << "{"
        ...
        << "\"kalman_prediction_enable\":" << (cfg.kalman_prediction_enable ? "true" : "false") << ","
        << "\"kalman_active\":" << (shared_state.kalman_prediction_enable ? "true" : "false") << ","
        << "\"kalman_residual_px\":" << shared_state.kalman_residual_px << ","
        << "\"kalman_max_residual_px\":" << shared_state.kalman_max_residual_px << ","
        << "\"kalman_prediction_age_ms\":" << shared_state.kalman_prediction_age_ms << ","
        << "\"kalman_predicted_only_frames\":" << shared_state.kalman_predicted_only_frames << ","
        << "\"kalman_snap_count\":" << shared_state.kalman_snap_count << ","
        ...
        << "\"detection_dampening_ready\":" << (shared_state.detection_dampening_ready ? "true" : "false") << ","
        << "\"detection_dampening_streak\":" << shared_state.detection_dampening_streak << ","
        << "\"detection_dampening_required_frames\":" << shared_state.detection_dampening_required_frames << ","
...
<body>
  ...
  <script>
    const fields = [
      ...
      {key:"display_rate_servo_enable",label:"Display Rate Servo",type:"bool"},
      {key:"display_rate_servo_hz",label:"Display Servo Hz",type:"number",step:1,min:0},
      {key:"display_rate_servo_max_target_age_ms",label:"Servo Max Target Age (ms)",type:"number",step:1,min:0},
      ...
      {key:"kalman_prediction_enable",label:"Kalman Prediction",type:"bool"},
      {key:"kalman_process_noise",label:"Kalman Process Noise",type:"number",step:0.001,min:0},
      {key:"kalman_measurement_noise",label:"Kalman Measurement Noise",type:"number",step:0.001,min:0},
...
body{margin:0;padding:18px;background:#0f1518;color:#eef6fa;font:14px "Segoe UI",sans-serif}main{max-width:1240px;margin:0 auto}section{background:#162126;border:1px solid #2b3b42;border-radius:16px;padding:16px;margin:0 0 16px}h1,h2,h3{margin:0 0 10px}.grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:10px}.field{display:grid;gap:6px}.toggle{display:flex;align-items:center;justify-content:space-between;gap:8px}.bar{padding:10px 12px;border:1px solid #2b3b42;border-radius:12px;background:#10181c;margin:0 0 10px;color:#c7d7de}.chip{padding:9px 10px;border:1px solid #34464f;border-radius:10px;background:#0c1215;color:#eef6fa}.tuning-groups{display:grid;grid-template-columns:repeat(auto-fit,minmax(320px,1fr));gap:12px}.config-group{border:1px solid #2b3b42;border-radius:14px;background:#10181c;padding:14px}.config-group h3{font-size:14px;letter-spacing:.02em;color:#f0b35a;margin-bottom:12px}.group-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:10px}input,select,a,button{box-sizing:border-box}input,select,a{width:100%;padding:9px 10px;border-radius:10px;border:1px solid #34464f;background:#0c1215;color:#eef6fa;text-decoration:none}button{padding:10px 14px;border:0;border-radius:999px;cursor:pointer}button.primary{background:#f0b35a;color:#1e1408;font-weight:700}button.secondary{background:#223038;color:#eef6fa;border:1px solid #34464f}.actions{display:flex;flex-wrap:wrap;gap:10px;margin-top:14px}
...
{g:"Aim & Selection",k:"body_y_ratio",l:"Body Aim Y Ratio",t:"n",s:0.01,n:0},{g:"Aim & Selection",k:"head_x_ratio",l:"Head Aim X Ratio",t:"n",s:0.01},{g:"Aim & Selection",k:"head_y_ratio",l:"Head Aim Y Ratio",t:"n",s:0.01,n:0},{g:"Aim & Selection",k:"tracking_strategy",l:"Tracking Strategy",t:"s",o:[["raw","Raw Detection"],["raw_delta","Raw + Velocity"],["legacy_pid","Legacy PID"],["predictive_pid","Predictive PID"]]},{g:"Aim & Selection",k:"detection_dampening_enable",l:"Detection Dampening",t:"b"},{g:"Aim & Selection",k:"detection_dampening_stable_frames",l:"Dampening Frames",t:"n",s:1,n:1},{g:"Aim & Selection",k:"kalman_prediction_enable",l:"Kalman Prediction",t:"b"},{g:"Aim & Selection",k:"kalman_process_noise",l:"Kalman Process Noise",t:"n",s:0.001,n:0},{g:"Aim & Selection",k:"kalman_measurement_noise",l:"Kalman Measurement Noise",t:"n",s:0.001,n:0},{g:"Aim & Selection",k:"sticky_bias_px",l:"Sticky Bias (px)",t:"n",s:1},{g:"Aim & Selection",k:"target_max_lost_frames",l:"Max Lost Frames",t:"n",s:1,n:1},
...
bool applyRuntimePatch(const std::string& body, RuntimeConfig& cfg, std::string& error) {
    ...
    if (const auto value = extractJsonBool(body, "async_gpu_capture_fresh_only_enable"); value.has_value()) {
        cfg.async_gpu_capture_fresh_only_enable = *value;
    }
    if (const auto value = extractJsonBool(body, "tensorrt_inline_fresh_only_enable"); value.has_value()) {
        cfg.tensorrt_inline_fresh_only_enable = *value;
    }
    ...
    if (const auto value = extractJsonNumber(body, "display_rate_servo_hz"); value.has_value()) {
        cfg.display_rate_servo_hz = std::max(0.0, *value);
    }
    if (const auto value = extractJsonNumber(body, "display_rate_servo_max_target_age_ms"); value.has_value()) {
        cfg.display_rate_servo_max_target_age_ms = std::max(0.0, *value);
    }
    if (const auto value = extractJsonBool(body, "detection_dampening_enable"); value.has_value()) {
        cfg.detection_dampening_enable = *value;
    }
    if (const auto value = extractJsonNumber(body, "detection_dampening_stable_frames"); value.has_value()) {
        cfg.detection_dampening_stable_frames = std::max(1, static_cast<int>(std::lround(*value)));
    }
    ...
    if (const auto value = extractJsonNumber(body, "predictive_pid_kp"); value.has_value()) cfg.predictive_pid_kp = static_cast<float>(*value);
    if (const auto value = extractJsonNumber(body, "predictive_pid_ki"); value.has_value()) cfg.predictive_pid_ki = static_cast<float>(*value);
    if (const auto value = extractJsonNumber(body, "predictive_pid_kd"); value.has_value()) cfg.predictive_pid_kd = static_cast<float>(*value);
    if (const auto value = extractJsonNumber(body, "predictive_pid_pred_weight_x"); value.has_value()) {
        cfg.predictive_pid_pred_weight_x = static_cast<float>(*value);
    }
    if (const auto value = extractJsonNumber(body, "predictive_pid_pred_weight_y"); value.has_value()) {
        cfg.predictive_pid_pred_weight_y = static_cast<float>(*value);
    }
    if (const auto value = extractJsonNumber(body, "predictive_pid_init_scale"); value.has_value()) {
        cfg.predictive_pid_init_scale = clamp(static_cast<float>(*value), 0.0F, 1.0F);
    }
    if (const auto value = extractJsonNumber(body, "predictive_pid_ramp_time_s"); value.has_value()) {
        cfg.predictive_pid_ramp_time_s = std::max(0.0F, static_cast<float>(*value));
    }
    if (const auto value = extractJsonNumber(body, "predictive_pid_integral_limit"); value.has_value()) {
        cfg.predictive_pid_integral_limit = std::max(0.0F, static_cast<float>(*value));
    }
    if (const auto value = extractJsonNumber(body, "predictive_pid_derivative_limit"); value.has_value()) {
        cfg.predictive_pid_derivative_limit = std::max(0.0F, static_cast<float>(*value));
    }
    if (const auto value = extractJsonNumber(body, "predictive_pid_output_limit"); value.has_value()) {
        cfg.predictive_pid_output_limit = std::max(0.0F, static_cast<float>(*value));
    }
    if (const auto value = extractJsonNumber(body, "predictive_pid_velocity_alpha"); value.has_value()) {
        cfg.predictive_pid_velocity_alpha = clamp(static_cast<float>(*value), 0.0F, 1.0F);
    }
    if (const auto value = extractJsonNumber(body, "predictive_pid_acceleration_alpha"); value.has_value()) {
        cfg.predictive_pid_acceleration_alpha = clamp(static_cast<float>(*value), 0.0F, 1.0F);
    }
    if (const auto value = extractJsonNumber(body, "predictive_pid_max_velocity_px_s"); value.has_value()) {
        cfg.predictive_pid_max_velocity_px_s = std::max(0.0F, static_cast<float>(*value));
    }
    if (const auto value = extractJsonNumber(body, "predictive_pid_max_acceleration_px_s"); value.has_value()) {
        cfg.predictive_pid_max_acceleration_px_s = std::max(0.0F, static_cast<float>(*value));
    }
    if (const auto value = extractJsonNumber(body, "predictive_pid_reverse_gate_px"); value.has_value()) {
        cfg.predictive_pid_reverse_gate_px = std::max(0.0F, static_cast<float>(*value));
    }
    if (const auto value = extractJsonNumber(body, "predictive_pid_reverse_scale"); value.has_value()) {
        cfg.predictive_pid_reverse_scale = clamp(static_cast<float>(*value), 0.0F, 1.0F);
    }
    if (const auto value = extractJsonNumber(body, "predictive_pid_prediction_error_scale"); value.has_value()) {
        cfg.predictive_pid_prediction_error_scale = std::max(0.0F, static_cast<float>(*value));
    }
    if (const auto value = extractJsonNumber(body, "predictive_pid_prediction_min_px"); value.has_value()) {
        cfg.predictive_pid_prediction_min_px = std::max(0.0F, static_cast<float>(*value));
    }
    if (const auto value = extractJsonNumber(body, "predictive_pid_prediction_max_px"); value.has_value()) {
        cfg.predictive_pid_prediction_max_px = std::max(cfg.predictive_pid_prediction_min_px, static_cast<float>(*value));
    }
    cfg.predictive_pid_prediction_max_px = std::max(cfg.predictive_pid_prediction_max_px, cfg.predictive_pid_prediction_min_px);
    if (const auto value = extractJsonBool(body, "predictive_pid_latency_comp_enable"); value.has_value()) {
        cfg.predictive_pid_latency_comp_enable = *value;
    }
    if (const auto value = extractJsonBool(body, "predictive_pid_latency_auto_enable"); value.has_value()) {
        cfg.predictive_pid_latency_auto_enable = *value;
    }
    if (const auto value = extractJsonNumber(body, "predictive_pid_latency_bias_s"); value.has_value()) {
        cfg.predictive_pid_latency_bias_s = std::max(0.0F, static_cast<float>(*value));
    }
    if (const auto value = extractJsonNumber(body, "predictive_pid_latency_max_s"); value.has_value()) {
        cfg.predictive_pid_latency_max_s = std::max(0.0F, static_cast<float>(*value));
    }
    if (const auto value = extractJsonBool(body, "predictive_pid_deadzone_enable"); value.has_value()) {
        cfg.predictive_pid_deadzone_enable = *value;
    }
    if (const auto value = extractJsonNumber(body, "predictive_pid_deadzone_enter_px"); value.has_value()) {
        cfg.predictive_pid_deadzone_enter_px = std::max(0.0F, static_cast<float>(*value));
    }
    if (const auto value = extractJsonNumber(body, "predictive_pid_deadzone_exit_px"); value.has_value()) {
        cfg.predictive_pid_deadzone_exit_px = std::max(cfg.predictive_pid_deadzone_enter_px, static_cast<float>(*value));
    }
    cfg.predictive_pid_deadzone_exit_px = std::max(cfg.predictive_pid_deadzone_exit_px, cfg.predictive_pid_deadzone_enter_px);
    if (const auto value = extractJsonNumber(body, "predictive_pid_deadzone_enter_ratio"); value.has_value()) {
        cfg.predictive_pid_deadzone_enter_ratio = clamp(static_cast<float>(*value), 0.0F, 1.0F);
    }
    if (const auto value = extractJsonNumber(body, "predictive_pid_deadzone_exit_ratio"); value.has_value()) {
    ...
    if (const auto value = extractJsonNumber(body, "detection_box_scale"); value.has_value()) {
        cfg.detection_box_scale = clamp(static_cast<float>(*value), 0.05F, 2.0F);
    }
    if (const auto value = extractJsonBool(body, "kalman_prediction_enable"); value.has_value()) {
        cfg.kalman_prediction_enable = *value;
    }
    if (const auto value = extractJsonNumber(body, "kalman_process_noise"); value.has_value()) {
        cfg.kalman_process_noise = std::max(0.0F, static_cast<float>(*value));
    }
    if (const auto value = extractJsonNumber(body, "kalman_measurement_noise"); value.has_value()) {
        cfg.kalman_measurement_noise = std::max(0.0F, static_cast<float>(*value));
    }
    ...
    if (const auto value = extractJsonNumber(body, "triggerbot_arm_scale_y"); value.has_value()) {
        cfg.triggerbot_arm_scale_y = std::max(0.0F, static_cast<float>(*value));
    }
    if (const auto value = extractJsonNumber(body, "triggerbot_arm_min_x_px"); value.has_value()) {
        cfg.triggerbot_arm_min_x_px = std::max(0, static_cast<int>(std::lround(*value)));
    }
    if (const auto value = extractJsonNumber(body, "triggerbot_arm_min_y_px"); value.has_value()) {
        cfg.triggerbot_arm_min_y_px = std::max(0, static_cast<int>(std::lround(*value)));
    }
    ...
    if (const auto value = extractJsonBool(body, "mouse_move_suppress_on_fire_enable"); value.has_value()) {
        cfg.mouse_move_suppress_on_fire_enable = *value;
    }
    if (const auto value = extractJsonBool(body, "mouse_move_suppress_on_fire_debug"); value.has_value()) {
        cfg.mouse_move_suppress_on_fire_debug = *value;
    }
