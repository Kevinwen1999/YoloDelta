...
class OnnxRuntimeBackend:
    def __init__(self, model_path, imgsz, conf, device, max_det):
        ...
        self.imgsz = imgsz
        self.input_h = int(imgsz)
        self.input_w = int(imgsz)
        self.conf = conf
        self.max_det = max_det
        self.device = device
        self.ort = ort
    ...
    def _build_providers(self, use_cuda_graph):
        providers = []
        if self.device == "cuda":
            if ONNX_USE_TENSORRT_EP and "TensorrtExecutionProvider" in self.available:
    ...
    def _init_session(self, use_cuda_graph):
        ...
        if len(input_shape) >= 4:
            ...
            w_dim = input_shape[3]
            if (
                isinstance(h_dim, (int, np.integer))
                and isinstance(w_dim, (int, np.integer))
                and int(h_dim) > 0
                and int(w_dim) > 0
            ):
                self.input_h = int(h_dim)
                self.input_w = int(w_dim)
        if self.input_h != int(self.imgsz) or self.input_w != int(self.imgsz):
            print(
                f"[INFO] ONNX input is fixed at {self.input_w}x{self.input_h}; "
                f"overriding IMGSZ={self.imgsz} for ONNX backend."
            )
    ...
    def _decode_nms_output(self, pred, frame_w, frame_h, target_cls):
        ...
        if coord_max <= (max(self.input_w, self.input_h) + 8):
...
def main():
    ...
    def inference_loop():
        ...
        while running:
            ...
            # Ported from test.py: classes are directly mapped by aimmode (0=head, 1=body).
            target_cls = local_aimmode
            ...
            if bbox is None:
                ...
                best_conf = 0.0
                try:
                    xyxy, cls_ids, confs = backend.predict(frame, target_cls)
                except Exception as e:
                    now_err = time.time()
                    ...
                    continue
                if xyxy.size > 0:
                    if cls_ids.size == confs.size and np.all(cls_ids == target_cls):
                        class_mask = confs >= DETECTION_MIN_CONF
                    else:
                        class_mask = (cls_ids == target_cls) & (confs >= DETECTION_MIN_CONF)
            ...
            dx = int(round(smooth_x - CENTER[0]))
            dy = int(round(smooth_y - CENTER[1]))
            deadzone = (
                AIM_DEADZONE_SLOW + ((AIM_DEADZONE_FAST - AIM_DEADZONE_SLOW) * speed_ratio)
                if local_use_kalman
                else 0.0
            )
            if abs(dx - last_out_dx) <= deadzone:
                dx = last_out_dx
            if abs(dy - last_out_dy) <= deadzone:
                dy = last_out_dy
            last_out_dx = dx
            last_out_dy = dy
            ...
            if DEBUG_LOG and (not used_tracker):
                print(
                    f"[DETECTED] cls={target_cls} at ({cx_full},{cy_full}) "
